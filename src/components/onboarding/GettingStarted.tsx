  ...
  useEffect(() => {
    const fetchStatus = async () => {
      ...
      const { data: profile } = await supabase
        ...
        .select('id, completion_bitmask, willo_company_key, knit_integration_id, knit_connected_at, knit_app_id, knit_category_id')
      ...
      const selectMostRecentBy = async (filter: { key: string; value: string; }) => {
      ...
      if (recent) {
        ...
        if (recent.willo_interview_key) {
          const { data: attempts } = await supabase
            ...
            .select('id')
            .eq('interview_id', recent.willo_interview_key)
            .limit(1);
...
useEffect(() => {
  ...
  .order('created_at', { ascending: false })
  .limit(1);
    return data?.[0]?.interview_link ?? null;
    };
