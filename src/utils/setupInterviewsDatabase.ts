...
export default async function setupInterviewsDatabase() {
  try {
    ...
    if (checkInterviewsError) {
      ...
      console.log(
        'Your admin should create the interviews table with the following columns:',
      );
      console.log(
        'id (UUID), user_id (UUID), title (TEXT), description (TEXT), language (TEXT)',
      );
      console.log(
        'hourly_rate (TEXT), questions (JSON), show_hints (BOOLEAN), show_availability (BOOLEAN)',
      );
      console.log(
        'deadline (TIMESTAMP), interview_link (TEXT), created_at (TIMESTAMP)',
      );
    } else {
    ...
    if (checkResponsesError) {
      ...
      console.log(
        'Note: The responses table should be created by an administrator',
      );
    } else {
