...
export default function Metrics() {
  ...
  // Add date picker modal component
  const DatePickerModal = () => {
    ...
    const handleApply = () => {
      ...
      // Trigger metrics refresh with new date range
      fetchMetricsData();
    };

    return (
      <div className={`fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 ${isDatePickerOpen ? '' : 'hidden'}`}>
  ...
  const handleExport = () => {
    ...
    const exportData = {
      ...
      detailed: metricsData.detailed
    };

    // Convert to CSV
    const csvContent = [
      // Headers
      ['Metric Type', 'Value', 'Change'],
      // Summary data
      ['Total Ad Spend', metricsData.summary.spend, '+12.3%'],
      ['Total Revenue', metricsData.summary.revenue, '+8.7%'],
      ['ROAS', metricsData.summary.roas, '+5.2%'],
      ['Conversion Rate', metricsData.summary.conversionRate, '-2.1%'],
      // Blank line
      [],
      // Detailed metrics headers
      ['Campaign', 'Spend', 'CPC', 'CTR', 'Conversions', 'CPA', 'Change'],
      // Detailed metrics data
      ...metricsData.detailed.map(metric => [
        metric.name,
        metric.spend,
        metric.cpc,
        metric.ctr,
    ...
    link.setAttribute('download', `metrics_export_${timeRange}.csv`);
      ...
      <html>
        ...
        <body>
          ...
          <table>
            ...
            <tr><td>Total Ad Spend</td><td>${metricsData.summary.spend}</td><td>+12.3%</td></tr>
            <tr><td>Total Revenue</td><td>${metricsData.summary.revenue}</td><td>+8.7%</td></tr>
            <tr><td>ROAS</td><td>${metricsData.summary.roas}</td><td>+5.2%</td></tr>
            <tr><td>Conversion Rate</td><td>${metricsData.summary.conversionRate}</td><td>-2.1%</td></tr>
          </table>
