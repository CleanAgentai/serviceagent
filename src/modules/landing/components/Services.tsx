    ...
    <section id="pricing" className="relative py-24 bg-gradient-to-br from-[#0B1C2D]/3 via-white to-[#A1E3FF]/8 overflow-hidden">
      ...
      <div className="relative z-10 container mx-auto px-6">
        <div className="text-center mb-16">
          ...
          <p className="text-xl text-slate-600 max-w-2xl mx-auto">Flexible plans designed to scale with your business. Choose the perfect fit and transform your hiring process today.</p>
        </div>
        ...
        <div className="grid md:grid-cols-3 gap-8 max-w-7xl mx-auto">
          {plans.map((plan, index) => (
            <Card key={index} className={`relative border-0 shadow-lg hover:shadow-xl transition-all duration-300 hover:-translate-y-2 ${plan.popular ? 'bg-gradient-to-br from-[#A1E3FF]/10 to-white ring-2 ring-[#0E7CFF]/30' : 'bg-white/80 backdrop-blur-sm'}`}>
              <CardContent className="p-8">
                ...
                {plan.cta === "Sign Up Now" ? (
                  <Link to={plan.ctaLink} className="w-full block">
                    <Button className="w-full bg-blue-600 hover:bg-blue-700 text-white py-3">{plan.cta}<ArrowRight className="ml-2 h-4 w-4" /></Button>
                  </Link>
                ) : (
                  <Button className="w-full bg-blue-600 hover:bg-blue-700 text-white py-3" onClick={() => window.open(plan.ctaLink, '_blank')}>{plan.cta}<ArrowRight className="ml-2 h-4 w-4" /></Button>
                )}
              </CardContent>
            </Card>
          ))}
